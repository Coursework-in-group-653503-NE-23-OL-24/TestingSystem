...
extern AnsiString usersurname;
extern AnsiString usergroup;
...
void __fastcall TFTest::ButtonAnswerClick(TObject *Sender)
{
	...
	if (wasChanged==false){
		   ...
		   /*	AnsiString cell;
			WayToFile=CURRENT_DIRECTORY+"\\Amounts.xlsx";// В файл просто добавляется оценка(для вычисления лучшей и худшей)
			int rowsCount1;
			...
			try{
				...
				ExcelBooks=ExcelApplication.OlePropertyGet("Workbooks").OlePropertyGet("Open",StringToOleStr(WayToFile));
				Sheet=ExcelBooks.OlePropertyGet("Worksheets",1);
				rowsCount1=Sheet.OlePropertyGet("UsedRange").OlePropertyGet("Rows").OlePropertyGet("Count");
			}
			catch(...){
				ShowMessage("Ошибка при обращении к файлу\n"+WayToFile);
				ExcelApplication.OleProcedure("Quit");
			...
			try{
				...
				cell=Sheet.OlePropertyGet("Cells").OlePropertyGet("Item",i-1,1);//Текст клетки A
				addToCell(Sheet,i-1,1,IntToStr(testStatistics.testResult()));
				ExcelApplication.OlePropertyGet("Workbooks").OlePropertyGet("Item",1).OleProcedure("Save");
				ExcelApplication.OleProcedure("Quit");
				//FStart->ComboBoxUsers->ItemIndex=0;//выбор первого пользователя по умолчанию
			}
...
void changeQuestion(int index){
	...
	FTest->RadioGroupVariants->Width=FTest->MemoQuestionText->Width;
	...
	if (TestQuestions[index].getQuestionType()=="video" && TestQuestions[index].getVideoResolutionX()>480) {
		FTest->ButtonAnswer->Left=FTest->PanelVideo->Left + FTest->PanelVideo->Width +3*INDENTION;
		FTest->RadioGroupVariants->Width=FTest->ClientWidth- FTest->MemoQuestionText->Left - FTest->PanelVideo->Width - 6*INDENTION;
